...
# Release formula template for model-governance-ledger
# =====================================================
# Copy this file to Formula/model-governance-ledger@__VERSION__.rb and replace
# every __PLACEHOLDER__ token with the real value from the official GitHub Release.
#
# Required inputs
# ---------------
...
#   https://github.com/SocioProphet/model-governance-ledger/releases
#
# Provenance / SBOM
# -----------------
...
class ModelGovernanceLedgerRelease < Formula
  ...
  def install
    ...
    (bin/"model-governance-ledger").write <<~EOS
      #!/usr/bin/env bash
      set -euo pipefail
      exec "#{python}" "#{libexec}/tools/model_governance_ledger.py" "$@"
    EOS
  end

  test do
    assert_match "model-governance-ledger", shell_output("#{bin}/model-governance-ledger --version 2>&1")
  end
end
